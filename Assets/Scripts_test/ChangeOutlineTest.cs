...
public class ChangeOutlineTest : MonoBehaviour
{
  ...
  // // [SerializeField] UwcWindow uwcWindow;

  UwcWindow uwcWindow;

  List<UwcWindow> uwcWindows;

  Texture2D uwcWindowTexture;
  //int pixelCount = 0;
  ...
  void Start()
  {
    ...
    // UwcWindow uwcWindow = UwcManager.Find("img_colorful.png");

    uwcWindows = UwcManager.FindAll("");

    uwcWindow = UwcManager.Find("img_colorful.png");
  ...
  void Update()
  {
    ...
    Debug.Log(uwcWindow);
    // uwcWindowTexture = uwcWindow.texture;
    ...
    // var window = uwcWindow;


    // Debug.Log(uwcWindow.height);
    ...
    {
      // outline.OutlineColor = uwcWindow.texture.GetPixel(pixelX, pixelY);
      //var cursorPos = Lib.GetCursorPosition();
      //var x = cursorPos.x - window.x;
      //var y = cursorPos.y - window.y;
      //material.color = window.GetPixel(x, y);
